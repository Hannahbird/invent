        ...
        <div className="container">
          {confirmed ? (
            <div className="justify-content-center">
              <p className="display-3 opacity-75">
                Your request was successfully submitted, the event coordinator
            ...
            <h2 className="text-center">
              Select and request a space for your event at {company.title}
            </h2>
          )}
        </div>
        {!confirmed && (
          <div className="container d-flex flex-wrap mt-3 justify-content-evenly">
            {locations.map((location) => (
              <Card
                className="mt-3 text-center d-flex justify-content-center col-lg-3 col-md-5"
                key={location._id}
              >
                {location.image && (
                  <Card.Img
                    ...
                    src={location.image.encodedImage}
                  />
                )}
                <Card.Body>
                  <Card.Title>{location.locationName}</Card.Title>
                  <Card.Text>Capacity: {location.capacity}</Card.Text>
                  <Button
                    variant="primary"
                    className="btn-warning col-8"
                    onClick={() => {
                      loadReserveForm(location._id, location.locationName);
                    }}
                  >
                    Request Space
                  </Button>
